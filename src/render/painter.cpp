...
// 字号物理下限保护：base 为 96dpi 基准字号，保证 逻辑字号×缩放 不小于 minPhys 物理像素，
// 小窗口下文字不被缩没（功德 ≥13px、目标 ≥11px）
float FloorFontPx(double base, double scale, double minPhys) {
    double px = base;
    if (px * scale < minPhys)
        px = minPhys / scale;
...
// 漂浮文字：出生点上浮 110px，金色/棕色，随生命周期淡出
void DrawFloats(Graphics &g, const FontFamily &fam, const AppState &st, ULONGLONG now) {
...
// 功德计数：金色大字，木鱼正上方居中
void DrawMeritCounter(Graphics &g, const FontFamily &fam, const AppState &st, float s) {
    float mpx = FloorFontPx(20.0, s, 13.0);
    Font f(&fam, mpx, FontStyleBold, UnitPixel);
    StringFormat sf;
...
void DrawGoalArea(Graphics &g, const FontFamily &fam, const AppState &st, float s,
                  ULONGLONG now) {
    ...
    if (st.daily >= gt) {
        // 达成：与佛光同拍呼吸的金色"功德圆满"
        float fpx = FloorFontPx(15.0, s, 13.0);
        Font f(&fam, fpx, FontStyleBold, UnitPixel);
    ...
    // 未达成：文字底边固定在进度条上方，字号被物理下限放大时向上生长，避免被裁
    float gpx = FloorFontPx(12.0, s, 11.0);
    Font f(&fam, gpx, FontStyleRegular, UnitPixel);
...
// 达成佛光：金色放射光线绕鱼身慢速旋转（90s 一转）+ 明暗呼吸（素材自带 alpha 通道）
void DrawGlow(Graphics &g, media::Assets &assets, ULONGLONG now) {
    ...
    g.RotateTransform(static_cast<REAL>(std::fmod(static_cast<double>(now) / 250.0, 360.0)));
    g.DrawImage(assets.glow, RectF(-225, -225, 450, 450), 0, 0, assets.glow->GetWidth(),
                assets.glow->GetHeight(), UnitPixel, &ia);
    g.Restore(gs);
}

// 音波纹：以触鱼点为中心的两圈椭圆扩散
void DrawRipples(Graphics &g, long long idt) {
...
void Render(HWND hwnd, const AppState &st, media::Assets &assets) {
...

    {
        ...
        g.TranslateTransform(static_cast<REAL>(-config::kFishCx),
                             static_cast<REAL>(-config::kFishCy));
        if (st.GoalTotal() > 0 && st.daily >= st.GoalTotal() && assets.glow)
            DrawGlow(g, assets, now);
        ...
        DrawFloats(g, fam, st, now);
        DrawMeritCounter(g, fam, st, s);
        DrawGoalArea(g, fam, st, s, now);
    }
